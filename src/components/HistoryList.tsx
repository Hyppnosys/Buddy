    ...
    return (
      <Card className="text-center py-10">
        <p className="text-sm text-(--color-ink-muted)">
          Nenhuma sessão ainda. Comece um período de foco para ver seu histórico aqui.
        </p>
      </Card>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      {Array.from(grouped.entries()).map(([dayKey, daySessions]) => (
        <div key={dayKey}>
          <h3 className="text-sm font-semibold text-(--color-ink-muted) mb-2">
            {formatDayLabel(daySessions[0].endedAt)}
          </h3>
            ...
            {daySessions.map((session) => (
              <div key={session.id} className="flex items-center gap-3 px-4 py-3">
                <span
                  ...
                  style={{ backgroundColor: TYPE_COLOR[session.type] }}
                  aria-hidden
                />
                <span className="text-sm font-mono text-(--color-ink-muted) w-14 shrink-0">
                  {formatClock(session.startedAt)}
                </span>
                <span className="text-sm font-medium flex-1 min-w-0 truncate">
                  {TYPE_LABEL[session.type]}
                </span>
                <span className="text-sm text-(--color-ink-muted) shrink-0">
                ...
                <span className="shrink-0" title={session.status === 'completed' ? 'Concluído' : 'Pulado'}>
                  {session.status === 'completed' ? (
                    ...
                    <XCircle size={16} className="text-(--color-ink-muted)" />
                  )}
                </span>
              </div>
            ))}
