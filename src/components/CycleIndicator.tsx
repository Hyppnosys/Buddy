...
export function CycleIndicator({ total, completed, isCurrentFocus }: CycleIndicatorProps) {
  return (
    <div
      ...
      role="img"
      aria-label={`Ciclo de foco: ${completed} de ${total} sessões concluídas`}
    >
        ...
        return (
          <span
            ...
            className={`rounded-full transition-all duration-300 ${
              isDone
                ? 'w-2.5 h-2.5 bg-(--color-focus)'
                : isCurrent
                  ? 'w-3 h-3 bg-(--color-focus) animate-soft-pulse'
                  : 'w-2.5 h-2.5 bg-(--color-border)'
            }`}
          />
        );
      })}
