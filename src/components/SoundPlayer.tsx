...
export function SoundPlayer() {
  ...
  const selectSound = (id: string) => {
    updateSettings((prev) => ({
      ...
      sound: {
        ...prev.sound,
        activeSoundId: prev.sound.activeSoundId === id ? null : id,
      },
    }));
  };

  const setVolume = (volume: number) => {
    updateSettings((prev) => ({ ...prev, sound: { ...prev.sound, volume } }));
  };
    ...
    <div className="flex flex-col gap-5">
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-2 rounded-2xl border p-4 transition-all duration-200
                ${
                  isActive
                    ? 'border-(--color-relax) bg-(--color-relax-soft) text-(--color-relax)'
                    : 'border-(--color-border) text-(--color-ink-muted) hover:border-(--color-relax) hover:text-(--color-relax)'
                }`}
            >
              <Icon size={20} />
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="w-11 h-11 shrink-0 rounded-full bg-(--color-relax) text-white flex items-center
            justify-center disabled:opacity-30 disabled:pointer-events-none transition-opacity"
        >
        ...
        <input
          ...
          className="flex-1 accent-(--color-relax)"
        />
      </div>

      {!activeSound && (
        <p className="text-xs text-(--color-ink-muted)">
          Escolha um som acima. Os arquivos de áudio podem ser adicionados futuramente — a estrutura já está pronta.
        </p>
      )}
    </div>
  );
