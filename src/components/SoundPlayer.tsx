import { CloudRain, Music, Pause, Play, Radio, Volume2, Waves } from 'lucide-react';
import { AMBIENT_SOUNDS } from '../utils/sounds';
import { useSettings } from '../hooks/useSettings';

const ICONS: Record<string, typeof CloudRain> = { CloudRain, Waves, Radio, Music };
...
export function SoundPlayer() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {!activeSound && <p className="text-xs text-(--color-ink-muted)">Escolha um som acima para tocá-lo.</p>}
      {activeSound && (
        <p className="text-xs text-(--color-ink-muted)">
          Esse som também toca durante a respiração e outras atividades de relaxamento, enquanto estiver ativado aqui.
        </p>
      )}
    </div>
  );
