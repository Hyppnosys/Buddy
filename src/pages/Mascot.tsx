...
export function Mascot() {
  ...
  const progressPercent = nextStage
    ? Math.min(100, (mascot.xp / (mascot.xp + nextStage.xpToGo)) * 100)
    : 100;
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 flex flex-col gap-6 animate-fade-up">
      <div>
        ...
        <p className="text-(--color-ink-muted) mt-1.5">
          Ele evolui conforme você (e seus amigos) completam atividades no app.
        </p>
      </div>
      ...
      <Card className="flex flex-col items-center gap-4 text-center py-8">
        ...
        {editingName ? (
          <div className="flex items-center gap-2">
            <input
              ...
              className="rounded-xl border border-(--color-border) bg-(--color-bg) px-3 py-1.5 text-sm text-center font-display font-semibold"
            />
            <button
              onClick={saveName}
              aria-label="Salvar nome"
              className="p-2 rounded-full bg-(--color-focus) text-white"
            >
              <Check size={14} />
            </button>
          </div>
          ...
          <button
            onClick={() => {
              setNameDraft(mascot.name);
              setEditingName(true);
            }}
            className="flex items-center gap-2 font-display text-xl font-semibold hover:text-(--color-focus) transition-colors"
          >
            {mascot.name}
        ...
        <span className="px-3 py-1 rounded-full bg-(--color-focus-soft) text-(--color-focus) text-xs font-semibold">
          Fase: {STAGE_LABEL[stage]}
        </span>

        <div className="w-full max-w-xs">
          <div className="h-2.5 rounded-full bg-(--color-surface-alt) overflow-hidden">
            <div
              className="h-full bg-(--color-focus) transition-all duration-500"
              style={{ width: `${progressPercent}%` }}
            />
          </div>
          <p className="text-xs text-(--color-ink-muted) mt-2">
            {nextStage
              ? `${nextStage.xpToGo} pontos para a fase "${STAGE_LABEL[nextStage.stage]}"`
          ...
          {COLOR_OPTIONS.map((color) => (
            <button
              ...
              style={{
                ...
                borderColor: mascot.color === color ? 'var(--color-ink)' : 'transparent',
              }}
            />
          ))}
        </div>
      ...
      <Card>
        ...
        <p className="text-sm text-(--color-ink-muted) mb-4">
          Amigos marcados aqui aparecem no registro de cuidados do mascote.
        </p>
        {friends.length === 0 ? (
          <p className="text-sm text-(--color-ink-muted)">
            Você ainda não tem amigos adicionados. Vá até a página Amigos para conectar alguém.
          </p>
        ) : (
          <div className="flex flex-col gap-2">
            {friends.map((friend) => {
              ...
              return (
                <button
                  key={friend.id}
                  onClick={() => toggleShareWithFriend(friend.id)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-colors
                    ${isShared ? 'border-(--color-focus) bg-(--color-focus-soft)' : 'border-(--color-border) hover:bg-(--color-surface-alt)'}`}
                >
                  <span
                    className="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-semibold shrink-0"
                    style={{ backgroundColor: friend.colorSeed }}
                  >
      ...
      <Card>
        ...
        {mascot.log.length === 0 ? (
          <p className="text-sm text-(--color-ink-muted)">
            Complete sessões de foco, diário, check-in ou yoga para ver o histórico aqui.
          </p>
        ) : (
              ...
              <li key={entry.id} className="flex items-center justify-between py-2.5 text-sm">
                <div>
                  ...
                  <p className="text-xs text-(--color-ink-muted)">
                    {entry.by} · {formatDayLabel(entry.at)} {formatClock(entry.at)}
                  </p>
                </div>
