      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-10 animate-fade-up">
        <Card className="text-center flex flex-col items-center gap-3 py-10">
          ...
          <p className="text-(--color-ink-muted) text-sm">
            Você já registrou como está hoje: {moodEmoji(todayCheckIn.mood)} {moodLabel(todayCheckIn.mood)}.
            Volte amanhã para um novo check-in.
          </p>
          {todayCheckIn.gratitude && (
            <p className="text-sm bg-(--color-surface-alt) rounded-xl px-4 py-3 mt-2 max-w-sm">
              "{todayCheckIn.gratitude}"
            </p>
          )}
        </Card>
  ...
  return (
    <div className="max-w-xl mx-auto px-4 sm:px-6 py-10 flex flex-col gap-6 animate-fade-up">
      ...
      <Card className="flex flex-col gap-6">
        ...
        <div>
          <label htmlFor="gratitude" className="text-sm font-medium block mb-1.5">
            Algo pelo qual você é grato(a) hoje? (opcional)
          </label>
          <textarea
            id="gratitude"
            value={gratitude}
        ...
        <Button onClick={handleSubmit} disabled={!mood} size="lg" className="self-start">
          Concluir check-in
        </Button>
      </Card>
    </div>
