...
function PersonAvatar({ name, avatarDataUrl, color }: { name: string; avatarDataUrl: string | null; color?: string }) {
  if (avatarDataUrl) {
    return <img src={avatarDataUrl} alt="" className="w-11 h-11 rounded-full object-cover shrink-0" />;
  }
  return (
    <div
      className="w-11 h-11 rounded-full flex items-center justify-center text-white font-semibold shrink-0"
      style={{ backgroundColor: color ?? 'var(--color-focus)' }}
      aria-hidden
    >
...
export function Friends() {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      setIsSearching(false);
    }, 250); // small delay so the "Buscando..." state is visible and typing doesn't thrash
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 flex flex-col gap-8 animate-fade-up">
      <div>
        ...
        <p className="text-(--color-ink-muted) mt-1.5">
          Busque outras pessoas cadastradas no app e conecte-se para cuidar do mascote junto.
        </p>
      </div>
      ...
      <Card>
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-(--color-border) bg-(--color-bg) pl-10 pr-4 py-2.5 text-sm"
          />
        </div>
        ...
        <div className="mt-4">
          {query.trim().length < 2 && (
            <p className="text-xs text-(--color-ink-muted)">
              Digite pelo menos 2 letras para buscar entre as contas cadastradas neste dispositivo.
            </p>
          )}
          ...
          {query.trim().length >= 2 && !isSearching && results !== null && results.length === 0 && (
            <p className="text-sm text-(--color-ink-muted)">Nenhum usuário encontrado.</p>
          )}
                ...
                return (
                  <div key={user.id} className="flex items-center gap-3 py-1.5">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 text-sm font-semibold text-(--color-focus)
                        disabled:text-(--color-ink-muted) disabled:cursor-default"
                    >
                      <UserPlus size={15} />
      ...
      {friends.length === 0 ? (
        <Card className="text-center py-10 flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-(--color-ink-muted)">
            Você ainda não tem amigos conectados. Use a busca acima para encontrar alguém.
          </p>
        </Card>
      ) : (
