      ...
      <div className="w-full max-w-4xl grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
        <div className="hidden lg:flex flex-col items-center text-center gap-4 px-6">
          ...
          <h2 className="font-display text-2xl font-semibold">
            {mode === 'login' ? 'Que bom te ver de novo' : 'Bem-vindo(a) ao Buddy'}
          </h2>
          <p className="text-(--color-ink-muted) max-w-xs">
            Seu mascote está te esperando para evoluir junto com sua rotina.
          </p>
        </div>
        ...
        <Card className="w-full max-w-md mx-auto">
          ...
          <div
            role="tablist"
            className="inline-flex items-center gap-1 p-1 rounded-full bg-(--color-surface-alt) mb-6 w-full"
          >
            <button
              role="tab"
              ...
              className={`flex-1 px-4 py-2 rounded-full text-sm font-semibold transition-all
                ${mode === 'login' ? 'bg-(--color-focus) text-white shadow-(--shadow-soft)' : 'text-(--color-ink-muted)'}`}
            >
              Entrar
            </button>
            <button
              ...
              className={`flex-1 px-4 py-2 rounded-full text-sm font-semibold transition-all
                ${mode === 'cadastro' ? 'bg-(--color-focus) text-white shadow-(--shadow-soft)' : 'text-(--color-ink-muted)'}`}
            >
              Criar conta
            </button>
          </div>
          ...
          <p className="text-xs text-(--color-ink-muted) text-center mt-5">
            Seus dados ficam salvos neste dispositivo/navegador.
          </p>

          <p className="text-center mt-4">
            <Link to="/" className="text-sm text-(--color-ink-muted) hover:text-(--color-ink) underline">
