    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 flex flex-col gap-8 animate-fade-up">
      <div>
        ...
        <p className="text-(--color-ink-muted) mt-1.5">
          Um espaço para registrar o seu dia, sem julgamentos.
        </p>
      </div>
      ...
      <Card className="flex flex-col gap-4">
        ...
        <textarea
          ...
          className="w-full rounded-xl border border-(--color-border) bg-(--color-bg) px-4 py-3 text-sm resize-none"
        />
        <Button onClick={handleSubmit} disabled={!content.trim() || !mood} className="self-start">
          Salvar no diário
        </Button>
      </Card>
        ...
        {entries.length === 0 && (
          <Card className="text-center py-10">
            <p className="text-sm text-(--color-ink-muted)">
              Nenhuma entrada ainda. Escreva a primeira acima.
            </p>
          </Card>
        )}
        {entries.map((entry) => (
          <Card key={entry.id} className="flex gap-3 items-start">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs text-(--color-ink-muted) mb-1">
                {formatDayLabel(entry.createdAt)} · {formatClock(entry.createdAt)}
              </p>
            ...
            <button
              ...
              className="shrink-0 p-1.5 rounded-full text-(--color-ink-muted) hover:text-(--color-danger) hover:bg-(--color-danger)/10 transition-colors"
            >
              <Trash2 size={15} />
            </button>
          </Card>
