...
export function useYogaSession(routine: YogaRoutine) {
  ...
  // Callback ref (em vez de um ref comum): o componente troca de bloco de
  // JSX ao iniciar/parar/concluir a rotina, e o React desmonta e recria o
  // elemento <video> a cada troca. Um ref comum só recebe o stream uma vez,
  // no primeiro elemento — no elemento novo, srcObject nunca é atribuído e a
  // câmera aparece preta. Esta função roda toda vez que um <video> (novo ou
  // antigo) é montado/desmontado, reconectando o stream já ativo.
  ...
  // Pede acesso à câmera assim que a tela da rotina abre, para a pessoa já
  // se ver e se posicionar antes de apertar "Começar rotina".
  useEffect(() => {
    let cancelled = false;
  ...
  // Carrega o modelo ONNX em paralelo à câmera, para já estar pronto quando o usuário começar.
  useEffect(() => {
    let cancelled = false;
    preloadModel()
      .then(() => {
  ...
  const runDetection = useCallback(async () => {
    ...
    if (detectionInFlightRef.current) return; // evita empilhar chamadas se um frame demorar mais que o intervalo
  ...
  // Avança para a próxima rotação (0° → 90° → 180° → 270° → 0°...). A pessoa
  // usa o botão de girar até se ver em pé no preview da câmera.
